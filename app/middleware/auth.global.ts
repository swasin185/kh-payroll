...
export default defineNuxtRouteMiddleware(async (to) => {
    ...
    if (to.path != loginUrl) {
        const { loggedIn, fetch: refreshUserSession, user } = useUserSession()
        ...
        if (menuLevel >= LEVELS.Entry && menuLevel <= LEVELS.Admin)
            $fetch("/api/permission/used", {
                method: "PUT",
                query: { comCode: user.value.comCode, userId: user.value.id, program: to.path },
            })
    }
})
