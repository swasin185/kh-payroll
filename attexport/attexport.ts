...
async function main() {
    ...
    try {
        ...
        const dateQuery: string = "select max(dateTxt) as maxDate from timecard where dateTxt <= curdate()";
        ...
        for (const record of checkInOutRecords) {
            ...
            if (batch.length >= BATCH_SIZE) {
                ...
                try {
                    const params = batch.flatMap(item => item); // Flatten the array for batch insertion
                    const placeholders = batch.map(() => '(?, ?, ?)').join(', ');
                    const batchInsertSQL = `INSERT INTO timecard (BadgeNumber, dateTxt, TimeTxt) VALUES ${placeholders} ON DUPLICATE KEY UPDATE BadgeNumber = VALUES(BadgeNumber)`; // Use ON DUPLICATE KEY UPDATE to mimic Java's ignored exception
        ...
        if (batch.length > 0) {
            try {
                ...
                const placeholders = batch.map(() => '(?, ?, ?)').join(', ');
                const batchInsertSQL = `INSERT INTO timecard (BadgeNumber, dateTxt, TimeTxt) VALUES ${placeholders} ON DUPLICATE KEY UPDATE BadgeNumber = VALUES(BadgeNumber)`;
