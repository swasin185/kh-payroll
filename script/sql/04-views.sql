...
select
    if(`v`.`morning` is not null and `v`.`evening` is not null and `v`.`night` is null, 'A. ปกติ', 'B. รอแก้ไข') AS `in_out_case`,
    if(`v`.`lunch_out` is null and `v`.`lunch_in` is null, '1. ไม่เช็คพักเที่ยง', if(`v`.`lunch_out` <> `v`.`lunch_in`, '2. มีพักเที่ยง', '3. สแกนครั้งเดียว')) AS `lunch_case`,
