...
select
    if(`v`.`morning` is not null and `v`.`evening` is not null, '1.เช้า-เย็น', if(`v`.`morning` is not null, '2.เช้าขาเดียว', if(`v`.`evening` is not null, '3.เย็นขาเดียว', '4.ไม่มี'))) AS `day_case`,
    if(`v`.`lunch_out` is null and `v`.`lunch_in` is null, '1.ไม่พักเที่ยง', if(`v`.`lunch_out` <> `v`.`lunch_in`, '2.มีพักเที่ยง', '3.สแกนครั้งเดียว')) AS `lunch_case`,
    if(`v`.`night` is null and `v`.`early` is null, '1.ไม่มีค่ำ', if(`v`.`early` is not null, '3.ข้ามวัน ', '2.ออกค่ำ')) AS `night_case`,
    ...
    case
        ...
        or `v`.`early` is not null) then 'Green'
        when `v`.`morning` is not null
        or (`v`.`evening` is not null
        or `v`.`night` is not null
        or `v`.`early` is not null)
        and (`v`.`lunch_out` is not null
        or `v`.`lunch_in` is not null) then 'Yellow'
        else 'Red'
    end AS `status`
from
    `vDailyTime` `v`;