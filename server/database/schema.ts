...
import { mysqlTable, foreignKey, boolean, varchar, tinyint, smallint, 
	 mediumint, decimal, index, unique, date, int, timestamp, year } from "drizzle-orm/mysql-core"
...
export const employee = mysqlTable("employee", {
	...
	timeCode: smallint().references(() => timetype.timeCode, { onDelete: "restrict", onUpdate: "restrict" }),
	// you can use { mode: 'date' }, if you want to have Date as type for this column
...
export const payroll = mysqlTable("payroll", {
	...
	comCode: varchar({ length: 2 }).notNull(),
	empCode: smallint().notNull(),
	inCode: varchar({ length: 2 }).notNull().references(() => incometype.inCode, { onDelete: "restrict", onUpdate: "restrict" }),
...
export const users = mysqlTable("users", {
	...
	comCode: varchar({ length: 2 }).default("01").notNull().references(() => company.comCode, { onDelete: "restrict", onUpdate: "restrict" }),
});
